...
while ( true )
    ...
    while ( true )
        ...
        if ( kcount <= icount ) % check on max number of fct calls
            break
        end
        ...
        for i = 2 : nn % get lowest fx & idx of lowest fx (LARGEST?)
            if ( ynewlo < y(i) )
                ynewlo = y(i);
                ihi = i;
            end
        ...
        if ( icount <= kcount )
            ...
            z = 0.0; % sum of squared deltas
            for i = 1 : nn
                z = z + ( y(i) - x )^2;
            end
            
            if ( z <= rq ) % termination criterion
                break
            end
    ...
    for i = 1 : n
        del = step(i) * eps;
        xmin(i) = xmin(i) + del; % small shift
        z = fn ( xmin ); % fx for shifted vertex xmin
        icount = icount + 1;
        if ( z < ynewlo ) % compare shifted fxmin' to fxmin 
            ifault = 2; % if better: break (fxmin was local minimum)
            break
        end
        xmin(i) = xmin(i) - del - del; % small shift
        z = fn ( xmin );
        icount = icount + 1;
        if ( z < ynewlo ) % compare shifted fxmin'' to fxmin
            ifault = 2; % if better: break (fxmin was local minimum)
    ...
    if ( ifault == 0 )
        break % global minimum assumed, terminate
    end
    %
    %  Restart the procedure.
    %
    for i = 1 : n
        start(i) = xmin(i); % set new starting point x0
    end
